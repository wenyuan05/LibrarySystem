...
- 支付宝 key 配置层兼容完整 PEM 和支付宝沙箱常见的一行 base64 key body：后端会自动补齐 `BEGIN/END PRIVATE KEY` 或 `BEGIN/END PUBLIC KEY` 包装并按 64 字符换行，避免 Node `crypto` 因 key 格式报 `DECODER routines::unsupported`。
